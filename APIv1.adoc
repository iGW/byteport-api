...
| device_uid::
 ...
 batch_register::
  Set this to True when using the range specifier when registering multiple devices using the dash.
 force::
  Registration will look like a successful new registration even if one or several devices already was existing.
 reg_code::
  OPTIONAL: A string representing the device publicly that can be used to bind to a Device, such as an EAN code. Used
  if you do not want to expose the Device UID publicly. Used by custom built applications mostly.
 active::
  Set to True if this device is considered to be an active in-use device right away.
...
....
Example 2) Registration failed due to invalid Device UID. Note, it still returns a 200 OK HTTP code.
....
{
