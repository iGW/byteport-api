...
#define FIX // Fixes so that a new line character is not required at the end of a frame
      ...
      // Check for content length
	  {
		  ...
		  if(content_length) {
			  ...
			  f->body = apr_pcalloc(pool, f->body_length);
                          ...
                          {
                              len = f->body_length;
                              rc = apr_socket_recv(connection->socket, f->body + offset, &len);
                              CHECK_SUCCESS;

			  if(f->body_length != len)
				printf("f->body_length = %d, len = %d\n", f->body_length, len);
